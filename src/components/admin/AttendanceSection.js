// construction/frontend/src/components/admin/AttendanceSection.js
import React, { useEffect, useState, useContext } from 'react';
import API from '../../api/axios';
import { toast } from 'react-toastify';
import Table from '../common/Table';
import Modal from '../common/Modal';
import AttendanceForm from '../supervisor/AttendanceForm'; // Reusing supervisor's form (it's generic)
import { FaPlus,FaClipboardCheck, FaEdit, FaTrash, FaFilter } from 'react-icons/fa';
import { AuthContext } from '../../context/AuthContext';

const AttendanceSection = ({ siteId }) => {
  const { user } = useContext(AuthContext); // Used for context, not for filtering in admin
  ...
  const fetchAttendanceEntries = async () => {
    ...
    try {
      ...
      const res = await API.get(`/attendance?${params}`); // Backend handles admin permissions
      setAttendanceEntries(res.data);
    } catch (error) {
      toast.error('Failed to load attendance entries for this project.');
  ...
  // Fetch workers specifically for this site (project)
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-sm">
      ...
      <Table
        ...
        emptyMessage="No attendance records found for this project with the selected criteria."
        renderRow={(entry) => (
